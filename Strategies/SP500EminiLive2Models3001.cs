...
{
    public class SP500EminiLive2Models3001 : Strategy
    {
        ...
        private static bool UseMaketViewExit = false;
        private static bool ConsultVServer = false;
        private static bool ExitOnSingleTrade = false;
        ...
        private void ReadConfigurationFile()
        {
            ...
            pathConfig = System.IO.Path.Combine(pathConfig, "Artista-Config-" + PortNumber.ToString() + ".xml");
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                Name = "SP500EminiLive2Models3001";
        ...
        protected override void OnOrderUpdate(Order order, double limitPrice, double stopPrice, int quantity, int filled, double averageFillPrice, OrderState orderState, DateTime time, ErrorCode error, string nativeError)
        {
            ...
            if (order.Name == "Long" || order.Name == "Short")
            {
                ...
                if (order.OrderState == OrderState.Filled || order.OrderState == OrderState.PartFilled)
                {
                    ...
                    // Reset currMarketView to Hold so that next five minutes no repeated trading
        ...
        private void AiFlat(ExitOrderType order)
        {
            ...
            if (!PosFlat())
            {
                ...
                // Mandatory halt trading after one trade
                if (ExitOnSingleTrade)
                    haltTrading = true;
            }
        }
        ...
        private bool IsTradeInterrupted()
        {
            ...
            if (PosLong())
            {
                ...
                // interrupt the trade when current exit view is opposite of current position
                if (UseMaketViewExit && currExitView == ExitView.Sell)
                {
                    MyPrint(defaultErrorType, "IsTradeInterrupted is True, exit via position opposite Exit View.");
                    profitPercentMet = false; // reset profitPercentMet flag
                    return true;
                }
            }
            if (PosShort())
            {
                ...
                // interrupt the trade when current exit view is opposite of current position
                if (UseMaketViewExit && currExitView == ExitView.Buy)
                {
                    MyPrint(defaultErrorType, "IsTradeInterrupted is True, exit via position opposite Exit View.");
                    profitPercentMet = false; // reset profitPercentMet flag
                    return true;
