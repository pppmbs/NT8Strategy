...
{
    public class AGG5MixIndicator : Strategy
    {
        private int Fast;
        private int Slow;
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description = @"AGG5 mixture of Bar and Tick strategy, using DLNN to manage start new position and stop loss, profit chasing depends on market trend - however use Bars.GetClose(CurrentBar) to determine market trend. Plus using indicator to confirm taking new position";
                Name = "AGG5MixIndicator";
        ...
        private void AiFlat()
        {
            ...
            if (PosLong())
            {
                ...
                Print("AiFlat::ExitLong");
                Print("---------------------------------------------------------------------------------");
            }
            if (PosShort())
            {
                ExitShort("ExitShort", "Short");
                Print("AiFlat::ExitShort");
                Print("---------------------------------------------------------------------------------");
            }
        ...
        private void StartTradePosition(string signal)
        {
            ...
            {
                case "0":
                    // sell if SMA(20) agrees
                    if (IsFalling(SMA(20)))
                        AiShort();
                    break;
                case "2":
                    // buy if SMA(20) agrees
                    if (IsRising(SMA(20)))
                        AiLong();
                    break;
                default:
